	...
	public void start() {
		try {
			...
			for (int i = 0; i < feeds.length; i++) {
				...
				for (Iterator<SyndEntryImpl> iterator = entries.iterator(); iterator
						.hasNext();) {
					SyndEntryImpl newsItem = iterator.next();
					if (lastUpdated == null
							|| lastUpdated.compareTo(
									newsItem.getPublishedDate()) < 0) {
						articleCounter++;
						NewsDocument newsDocument = new NewsDocument(
								feeds[i].getFeedName(), newsItem.getTitle(),
								newsItem.getLink(), newsItem.getDescription()
										.getValue(),
